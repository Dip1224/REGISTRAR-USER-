import { supabase, PERSONAS_TABLE, ROSTROS_BUCKET } from '../lib/supabaseClient.js';
...
async function iniciarCamara() {
    try {
        ...
        const tipoCamara = esMobil ? (usarCamaraFrontal ? 'frontal' : 'trasera') : `${camaraActual + 1}`;
        mostrarMensaje(`Camara ${tipoCamara} iniciada correctamente`, 'info');
        setTimeout(() => mensaje.style.display = 'none', 3000);
    } catch (error) {
        console.error('Error al iniciar cámara:', error);
        mostrarMensaje('No se pudo acceder a la camara. Error: ' + error.message, 'error');
    }
}
...
btnCapturar.addEventListener('click', () => {
    ...
    deteccionInfo.innerHTML = '<strong>Foto capturada correctamente</strong><br>Ingresa el nombre y haz clic en Registrar';
...
btnRegistrar.addEventListener('click', async () => {
    ...
    try {
        ...
        mostrarMensaje(error.message || 'No se pudo registrar a la persona', 'error');
        btnRegistrar.disabled = false;
        btnRegistrar.textContent = 'Registrar Persona';
    }
});
